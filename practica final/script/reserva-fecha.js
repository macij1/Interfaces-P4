          ...
          var fecha = new Date(anio, mes, diaActual);
            if (fecha < fechaActualReal || fechasReservadas.includes(fecha.toDateString())){
                ...
                celda.addEventListener("click", function() {
                  ...
                  var fechaSeleccionada = new Date(fechaActual.getFullYear(), fechaActual.getMonth(), diaSeleccionado);
      
                  if (fechaSeleccionada > fechaActualReal) {
                      setCookie("fechaSeleccionada", fechaSeleccionada.toDateString()); 
      
                      var fechasReservadas = obtenerFechasReservadas();
                      fechasReservadas.push(fechaSeleccionada.toDateString());
                      setCookie("fechasReservadas", JSON.stringify(fechasReservadas), 60*60*24*30*3); //expira en 3 meses 
